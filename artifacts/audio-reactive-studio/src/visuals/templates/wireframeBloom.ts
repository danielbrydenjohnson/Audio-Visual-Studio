    ...
    onFrame(time, _dt, audio) {
      ...
      for (const f of forms) {
        ...
        f.seg.rotation.set(
          ...
          f.rotSZ * time * rs * 0.95,
        );
        // LOW expands this form locally; the group is never scaled as one.
        const s = f.baseScale * esize * (1 + lowR * 0.55 + flick * 0.25);
        f.seg.scale.set(s, s, s);
        // Independent forward/back drift, clamped inside the depth volume.
        const z = f.posZ + Math.sin(time * f.driftSpeed + f.driftPhase) * f.driftAmp + lowR * 10.0;
        f.seg.position.set(
          f.posX,
        ...
        // Colour: palette base × per-form brightness (glow + LOW + MID + HIGH flicker).
        paletteMix(f.cmix, a, b, c, tmpCol);
        const bright = 0.55 + glow * 0.7 + lowR * 0.3 + midR * 0.25 + flick * 1.5;
        f.mat.color.setRGB(tmpCol.r * bright, tmpCol.g * bright, tmpCol.b * bright);
        f.mat.opacity = Math.min(1, 0.5 + lowR * 0.3 + flick * 0.5 + glow * 0.2);
      }
    },
